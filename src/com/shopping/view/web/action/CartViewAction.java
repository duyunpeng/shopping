			...
			if (CommUtil.null2String(cart_session1).equals(cart_session)) {
				for(StoreCart storeCart:cart){
					...
					if(null == area){
						mv = new JModelAndView("error.html", this.configService.getSysConfig(),
								this.userConfigService.getUserConfig(), 1, request, response);
						mv.addObject("op_title", "请先设置所在地区区域");
						mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
					}
					if(null != area){
						if(!(storeCart.getStore().getStore_name().equals("平台自营店")) || !(storeCart.getStore().getArea().getId().equals(area.getId()))){
							mv = new JModelAndView("error.html", this.configService.getSysConfig(),
									this.userConfigService.getUserConfig(), 1, request, response);
							if( (shopping_view_type != null) && (!shopping_view_type.equals( "" )) && (shopping_view_type.equals( "wap" )) ) {
								mv = new JModelAndView("wap/error.html", this.configService.getSysConfig(),
							...
							mv.addObject("op_title", "你不属于该区域，不能购买该区域的商品");
							mv.addObject("url", CommUtil.getURL(request) + "/index.htm");
						}
					}
				}
    ...
    public void wxcodepay(HttpServletRequest request, HttpServletResponse response, String order_id){
		...
		if (of.getOrder_status() == 10) {
			...
			if (returnCode.equalsIgnoreCase("SUCCESS")
				...
				String urls = request.getContextPath()+this.configService.getSysConfig().getUploadFilePath()
