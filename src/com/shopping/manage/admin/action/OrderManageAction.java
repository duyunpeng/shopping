 ...
 public class OrderManageAction
 {
   ...
   @SecurityMapping(display = false, rsequence = 0, title="配送列表", value="/admin/distribution_manage_list.htm*", rtype="admin", rname="配送管理", rcode="order_admin", rgroup="首页")
   @RequestMapping({"/admin/distribution_manage_list.htm"})
   public ModelAndView distribution_manage(HttpServletRequest request, HttpServletResponse response, String type, String type_data, String payment,String addr_id, String beginTime, String endTime, String begin_price, String end_price, String currentPage)
   {
     ModelAndView mv = new JModelAndView("admin/blue/distribution_manage_list.html",
             this.configService.getSysConfig(), this.userConfigService
             .getUserConfig(), 0, request, response);
     OrderFormQueryObject ofqo = new OrderFormQueryObject(currentPage, mv,
             "addTime", "desc");
       ofqo.addQuery("obj.order_status",
               new SysMap("order_status",
                       Integer.valueOf(CommUtil.null2Int(20))), "=");
     ...
     if (!CommUtil.null2String(type_data).equals("")) {
       ...
       if(!CommUtil.null2String(addr_id).equals("")){
         ofqo.addQuery("obj.addr.id", new SysMap("addr_id",Long.parseLong(addr_id)),"=");
       }
   ...
   @SecurityMapping(display = false, rsequence = 0, title="添加配送", value="/admin/distribution_add.htm*", rtype="admin", rname="配送管理", rcode="order_admin", rgroup="首页")
   @RequestMapping({"/admin/distribution_add.htm"})
   public ModelAndView distribution_add(HttpServletRequest request,HttpServletResponse response, String mulitId, String currentPage){
     ModelAndView mv = new JModelAndView("admin/blue/distribution_add.html",
             this.configService.getSysConfig(), this.userConfigService
             .getUserConfig(), 0, request, response);
     List orderFroms = new ArrayList();
     String[] ids = mulitId.split(",");
     for(String id:ids){
       if(!id.equals("")){
         OrderForm orderForm = this.orderFormService.getObjById(CommUtil.null2Long(id));
         orderFroms.add(orderForm);
       }
     }
  long area_id = 0;
     if(null != ids && ids.length!=0){
       area_id = this.orderFormService.getObjById(CommUtil.null2Long(ids[0])).getAddr().getArea().getId();
     }
     //查询该区域的配送司机
     Map map = new HashMap();
     map.put("area_id",area_id);
     List drivers = this.driverService.query( "select obj from Driver obj where obj.area.id=:area_id", map, -1, -1 );
     mv.addObject("drivers", drivers);
     mv.addObject("objs",orderFroms);
     mv.addObject("count",orderFroms.size());
     mv.addObject("mulitId",mulitId);
     return mv;
   }

   @SecurityMapping(display = false,rsequence = 0,title = "给司机配送订单",value = "/admin/driver_distribution.htm*",rtype = "admin",rname = "配送管理",rcode = "order_admin",rgroup = "首页")
   @RequestMapping("/admin/driver_distribution.htm")
   public String driver_distribution(HttpServletRequest request, HttpServletResponse response, String mulitId, String driver_id){
     List orderFroms = new ArrayList();
     String[] ids = mulitId.split(",");
     for(String id:ids){
       if(!id.equals("")){
     ...
     Driver driver = this.driverService.getObjById(CommUtil.null2Long(driver_id));
     driver.setOfs(orderFroms);
     this.driverService.update(driver);
     return "redirect:distribution_manage_list.htm";
   }
 }
